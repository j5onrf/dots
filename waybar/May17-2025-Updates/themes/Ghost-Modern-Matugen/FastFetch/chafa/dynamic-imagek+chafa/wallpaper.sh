...
if [ -f "$THEME_LOGO_SCRIPT_PATH" ]; then
    echo ":: wallpaper.sh: Calling theme_fastfetch_logo.sh" >&2
    (bash "$THEME_LOGO_SCRIPT_PATH" >> /tmp/theme_logo_debug.log 2>&1) & # Run in background, log output
else
    echo ":: wallpaper.sh: Logo theming script not found at $THEME_LOGO_SCRIPT_PATH" >&2
fi
# --- End of added block ---

echo ":: Wallpaper script finished." # This should now be the very last echo