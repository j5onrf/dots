...
# These ensure that even if a preferred specific color is missing, a sensible more general color is used.
# The final fallback is to PRIMARY or ON_BACKGROUND (FG) or BACKGROUND (BG) which *should* always exist.
# Or, as a last resort, a hardcoded hex if absolutely necessary (e.g., #000000 or #FFFFFF)
...
CUSTOMIZATIONS_JSON=$(cat <<EOF
{
  "workbench.colorCustomizations": {
    ...
    "editorIndentGuide.background": "#${OUTLINE_VARIANT:-${ON_SURFACE_VARIANT:-${FG:-c0c0c0}}20}",
    "editorIndentGuide.activeBackground": "#${OUTLINE:-${PRIMARY:-${SURFACE_TINT:-a8c8ff}}70}",
...
if [ -n "$CUSTOMIZATIONS_JSON" ] && jq -e . >/dev/null 2>&1 <<<"$CUSTOMIZATIONS_JSON"; then
    if [ -f "$VSCODIUM_SETTINGS_FILE" ]; then
        ...
        jq -s '.[0] * .[1]' "$VSCODIUM_SETTINGS_FILE" <(echo "$CUSTOMIZATIONS_JSON") > tmp_settings.json && mv tmp_settings.json "$VSCODIUM_SETTINGS_FILE"
    else
        echo "$CUSTOMIZATIONS_JSON" > "$VSCODIUM_SETTINGS_FILE"
    fi
    echo "VSCodium theme colors updated based on $CSS_FILE_TO_PARSE."
else
    echo "Error: CUSTOMIZATIONS_JSON was empty or invalid. VSCodium settings not updated."
    echo "Problematic JSON content was:"
    echo "$CUSTOMIZATIONS_JSON" # This will print the JSON with variables expanded
fi
