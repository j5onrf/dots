...
if [ ! -f "$MATUGEN_LOCK_FILE" ]; then
    ...
    # --- DYNAMICALLY GENERATE WAYBAR COLORS ---
    WAYBAR_DIR="$HOME/.config/waybar"
    COLOR_FILE="$WAYBAR_DIR/colors.css"
    DYNAMIC_CSS_FILE="$WAYBAR_DIR/dynamic-waybar-colors.css"
    HEX_COLOR=$(grep "@define-color primary_container" "$COLOR_FILE" | awk '{print $3}')
    ...
    if [ -n "$HEX_COLOR" ]; then
        ...
        G=$((16#${HEX_CLEAN:2:2}))
        B=$((16#${HEX_CLEAN:4:2}))
        echo "/* This file is auto-generated by wallpaper.sh. DO NOT EDIT. */" > "$DYNAMIC_CSS_FILE"
        echo "@define-color ghost_ws_button_active_bg_alpha25 rgba($R, $G, $B, 0.25);" >> "$DYNAMIC_CSS_FILE"
        echo "@define-color ghost_ws_button_active_bg_alpha40 rgba($R, $G, $B, 0.4);" >> "$DYNAMIC_CSS_FILE"
    fi
